...
public class KWoodcutter extends Script {
    ...
    private static final double NORMAL_XP = 25.0;
    ...
    private static final double OAK_XP = 37.5;
    ...
    private static final double DRAYNOR_WILLOW_XP = 67.5;
    ...
    private static final double BA_WILLOW_XP = 67.5;
    ...
    private static final double MAPLE_XP = 100.0;
    ...
    private double currentXPPerLog;
    ...
    private void createModernGUI() {
        ...
        startBtn.addActionListener(e -> {
            ...
            if (selectedTree.contains("Normal trees")) {
                ...
                currentXPPerLog = DRAYNOR_WILLOW_XP;
    ...
    public int poll() {
        ...
        // Update logs gained (only when dropping)
        int currentLogs = getCurrentLogCount();
        if (currentLogs < lastLogCount) {
            totalLogsGained += (lastLogCount - currentLogs);
        }
        lastLogCount = currentLogs;
        // Check chat for full inventory message to trigger early drop
        ...
        if (isDropping) {
            currentState = "Dropping";
            if (currentLogs == 0 && getClueBoxCount() == 0) {
                isDropping = false;
                currentChoppingTree = null;
                currentState = "Chopping";
                return random(600, 1200);
        ...
        currentState = "Chopping";
        if (currentLogs >= FULL_THRESHOLD || getClueBoxCount() > 0) {
            currentState = "Dropping";
            isDropping = true;
            currentChoppingTree = null;
            getWidgetManager().getInventory().dropItems(Set.of(currentLogID));
        ...
        if (!submitHumanTask(() -> !getUncutTrees().contains(tree), 20000)) { // Reduced to 20s
            log("Stump wait timed out after 20s - moving to next tree");
            currentChoppingTree = null;
            return random(600, 1200);
        }
        currentChoppingTree = null;
    ...
    public void onPaint(Canvas c) {
        ...
        double xpGained = totalLogsGained * currentXPPerLog;
        double xpPerHour = elapsedSeconds > 0 ? (xpGained * 3600000.0 / elapsed) : 0;
        int bgX = 10;
        int bgY = 20;
        int bgWidth = 280;
        int bgHeight = 180;
        ...
        textY += 35;
        c.drawText("Runtime: " + runtime, textX + 1, textY + 1, 0x66000000, statFont);
        c.drawText("Runtime: " + runtime, textX, textY, 0xFFFFFFFF, statFont);
        textY += 35;
        c.drawText(String.format("XP/hr: %.0f", xpPerHour), textX + 1, textY + 1, 0x66000000, statFont);
        c.drawText(String.format("XP/hr: %.0f", xpPerHour), textX, textY, 0xFF00FF00, statFont);
    }
    public void onExit() {
